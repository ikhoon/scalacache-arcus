...
object ArcusClientFactory {
  ...
  def apply(
    ...
    val connectionFactoryBuilder = getConnectionFactoryBuilder(
      enableFrontCache, frontCacheMaxElements, frontCacheExpireSecond, operateQueueSize,
      operateTimeout, maxReconnectDelay, readBufferSize, transcoderCharset, transcoderCompressionThreshold
    )
  ...
  private def getConnectionFactoryBuilder(
    enableFrontCache: Boolean,
